...
def construct_dataset(env, policy, max_episode_length, n_samples, privileged):
    ...
    for _ in range(n_samples):
        ...
        while True:
            ...
            if privileged:
                ...
                action = policy.action(obs, reward, done)
...
def evaluate_policy(env, policy, n_samples, privileged):
    ...
    for _ in range(n_samples):
        ...
        while not done:
            if privileged:
                action = policy.action(info["state"])
            else:
                action = policy.action(obs, reward, done)
...
def pad_episodes(episodes):
    ...
    # (o0, r0, d0, ...)
    assert np.min(lengths) >= 3

    # (a_i, o_i, r_i, d_i)
    dummy_aord = [
        torch.zeros_like(episodes[0][0]),
        torch.zeros_like(episodes[0][0]),
        torch.zeros_like(episodes[0][1]),
        torch.ones_like(episodes[0][2])]

    # padding
    max_length = np.max(lengths)
    for episode in episodes:
        while len(episode) < max_length:
            episode.extend(dummy_aord)

def collate_fn_pad_episodes(batch):
    pad_episodes([episode for _, episode in batch])

    return torch.utils.data.default_collate(batch)

def collate_fn_pad_episodes_weights(batch):
    pad_episodes([episode for (_, episode), _ in batch])
